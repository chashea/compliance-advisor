...
export default function Audit() {
  ...
  const { data, loading, error } = useApi<AuditResponse>("audit", department ? { department } : {}, [department]);
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold text-slate-800">Audit Log ({data.records.length} records)</h2>

      {data.service_breakdown.length > 0 && (
        <div>
          <h3 className="mb-2 text-sm font-medium text-slate-600">By Service</h3>
      ...
      {data.operation_breakdown.length > 0 && (
        <div>
          <h3 className="mb-2 text-sm font-medium text-slate-600">Top Operations</h3>
