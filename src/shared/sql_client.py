...
def upsert_control_scores(
    conn: sqlite3.Connection, tenant_id: str, snapshot_date: str, controls: list[dict]
) -> None:
    cursor = conn.cursor()
...
def upsert_control_profiles(
    conn: sqlite3.Connection, tenant_id: str, profiles: list[dict]
) -> None:
    cursor = conn.cursor()
    for p in profiles:
...
def upsert_benchmarks(
    conn: sqlite3.Connection, tenant_id: str, snapshot_date: str,
...
def mark_tenant_synced(conn: sqlite3.Connection, tenant_id: str) -> None:
    conn.cursor().execute(
        "UPDATE tenants SET last_synced_at = datetime('now') WHERE tenant_id = ?",
        (tenant_id,)
    )
    conn.commit()
...
def upsert_assessment(
    conn: sqlite3.Connection, tenant_id: str, assessment: dict,
) -> None:
    cursor = conn.cursor()
    cursor.execute("""
...
def upsert_assessment_control(
    conn: sqlite3.Connection, tenant_id: str, assessment_id: str, control: dict,
) -> None:
    cursor = conn.cursor()
    cursor.execute("""
